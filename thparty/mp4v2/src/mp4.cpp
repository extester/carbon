...
#if 1
static MP4FileWithProvider* ConstructMP4FileWithProvider()
{
    MP4FileWithProvider* pFile = NULL;
    try {
        pFile = new MP4FileWithProvider();
    }
    catch( std::bad_alloc ) {
        mp4v2::impl::log.errorf("%s: unable to allocate MP4File", __FUNCTION__);
    }
...
MP4FileHandle MP4CreateWithProvider(
    const char* fileName,
    ...
    uint32_t    compatibleBrandsCount)
{
        ...
        MP4FileWithProvider* pFile = ConstructMP4FileWithProvider();
        if (!pFile)
            return MP4_INVALID_FILE_HANDLE;
        ...
        try {
            ...
            pFile->CreateWithProvider(fileName, provider, flags,  add_ftyp, add_iods,
                          majorBrand, minorVersion,
                          compatibleBrands, compatibleBrandsCount);
